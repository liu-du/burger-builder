...
describe("<NavigationItems />", () => {
  ...
  it("should render 2 <NavigationItem /> if not authenticated", () => {
    ...
    expect(
      wrapper.contains(<NavigationItem link="/">Burger Builder</NavigationItem>)
    ).toEqual(true);
    expect(
  ...
  it("should render 3 <NavigationItem /> if not authenticated", () => {
    wrapper.setProps({ isAuth: true });
    expect(wrapper.find(NavigationItem)).toHaveLength(3);
  });

  it("should contains logout when authenticated", () => {
    wrapper.setProps({ isAuth: true });
    expect(
